...
function PageToggle({ label, hint, enabled, onToggle }: {
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
      ...
      <button
        type="button"
        onClick={onToggle}
        className={`relative w-14 h-7 rounded-full transition-colors ${enabled ? 'bg-green-500' : 'bg-gray-300'}`}
      >
        <span className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow transition-transform ${enabled ? 'left-8' : 'left-1'}`} />
      </button>
    </div>
  );
}

function PageContentEditor({ titleField, contentField, titleValue, contentValue, titlePlaceholder, contentPlaceholder, footerNote, defaultContent, onChange }: {
  titleField: keyof PageSettings; contentField: keyof PageSettings;
  titleValue: string; contentValue: string;
  titlePlaceholder: string; contentPlaceholder: string; footerNote: string;
    ...
    <>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          <button
            type="button"
            onClick={() => onChange(contentField, defaultContent)}
        ...
        />
        <p className="text-xs text-gray-500 mt-1">{footerNote}</p>
      </div>
    </>
  );
}
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6">
          {activeTab === 'contact' && (
            <div className="space-y-6">
              ...
              <PageToggle
                ...
                hint="Display contact page link in footer"
                enabled={formData.contact_page_enabled}
                onToggle={() => handleChange('contact_page_enabled', !formData.contact_page_enabled)}
              />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Page Title</label>
                  <input type="text" value={formData.contact_page_title} onChange={(e) => handleChange('contact_page_title', e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-gray-900" placeholder="Contact Us" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Subtitle</label>
                  <input type="text" value={formData.contact_page_subtitle} onChange={(e) => handleChange('contact_page_subtitle', e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-gray-900" placeholder="Have a question? We'd love to hear from you!" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1"><Mail size={14} className="inline mr-1" />Main Contact Email *</label>
                  <input type="email" value={formData.contact_email} onChange={(e) => handleChange('contact_email', e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-gray-900" placeholder="your-real-email@domain.com" required />
                  <p className="text-xs text-gray-500 mt-1">Your primary contact email</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Support Email</label>
                  <input type="email" value={formData.support_email} onChange={(e) => handleChange('support_email', e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-gray-900" placeholder="support@domain.com (optional)" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Business Email</label>
                  <input type="email" value={formData.business_email} onChange={(e) => handleChange('business_email', e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-gray-900" placeholder="business@domain.com (optional)" />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                ...
                <button type="button" onClick={() => handleChange('contact_phone_enabled', !formData.contact_phone_enabled)} className={`relative w-14 h-7 rounded-full transition-colors ${formData.contact_phone_enabled ? 'bg-green-500' : 'bg-gray-300'}`}>
              ...
              {formData.contact_phone_enabled && (
                <div>
                  <input type="tel" value={formData.contact_phone} onChange={(e) => handleChange('contact_phone', e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-gray-900" placeholder="+1 (XXX) XXX-XXXX" />
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                ...
                <button type="button" onClick={() => handleChange('contact_address_enabled', !formData.contact_address_enabled)} className={`relative w-14 h-7 rounded-full transition-colors ${formData.contact_address_enabled ? 'bg-green-500' : 'bg-gray-300'}`}>
              ...
              {formData.contact_address_enabled && (
                <div>
                  <textarea value={formData.contact_address} onChange={(e) => handleChange('contact_address', e.target.value)} rows={3} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-gray-900" placeholder={"123 Real Street\nCity, State 12345\nCountry"} />
          ...
          {activeTab === 'about' && (
            <div className="space-y-6">
              <PageToggle label="Show About Page" hint="Display About link in footer" enabled={formData.about_page_enabled} onToggle={() => handleChange('about_page_enabled', !formData.about_page_enabled)} />
              <PageContentEditor
                titleField="about_page_title" contentField="about_page_content"
                titleValue={formData.about_page_title} contentValue={formData.about_page_content}
                titlePlaceholder="About Us" contentPlaceholder="<h2>Our Story</h2>&#10;<p>Write your about page content here...</p>"
                footerNote="You can use HTML tags for formatting. Leave empty to use default content."
                defaultContent={defaultAboutContent} onChange={handleChange}
              />
            </div>
          )}
          ...
          {activeTab === 'privacy' && (
            <div className="space-y-6">
              <PageToggle label="Show Privacy Policy" hint="Display Privacy Policy link in footer" enabled={formData.privacy_page_enabled} onToggle={() => handleChange('privacy_page_enabled', !formData.privacy_page_enabled)} />
              <PageContentEditor
                titleField="privacy_page_title" contentField="privacy_page_content"
                titleValue={formData.privacy_page_title} contentValue={formData.privacy_page_content}
                titlePlaceholder="Privacy Policy" contentPlaceholder="<h2>Privacy Policy</h2>&#10;<p>Your privacy policy content here...</p>"
                footerNote="Leave empty to use default content. Consider consulting a lawyer for proper privacy policy."
                defaultContent={defaultPrivacyContent} onChange={handleChange}
              />
            </div>
          )}
          ...
          {activeTab === 'terms' && (
            <div className="space-y-6">
              <PageToggle label="Show Terms of Service" hint="Display Terms link in footer" enabled={formData.terms_page_enabled} onToggle={() => handleChange('terms_page_enabled', !formData.terms_page_enabled)} />
              <PageContentEditor
                titleField="terms_page_title" contentField="terms_page_content"
                titleValue={formData.terms_page_title} contentValue={formData.terms_page_content}
                titlePlaceholder="Terms of Service" contentPlaceholder="<h2>Terms of Service</h2>&#10;<p>Your terms content here...</p>"
                footerNote="Leave empty to use default content. Consider consulting a lawyer for proper terms."
                defaultContent={defaultTermsContent} onChange={handleChange}
              />
            </div>
          )}

          <div className="mt-8 flex justify-end">
            <button type="submit" disabled={saving} className="flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 font-medium">
              {saving ? (
                <><div className="animate-spin rounded-full h-5 w-5 border-t-2 border-white"></div><span>Saving...</span></>
              ) : (
