...
const getApiUrl = () => {
  ...
  if (host !== 'localhost' && host !== '127.0.0.1') return PRODUCTION_API_URL;
...
async function getArticle(slug: string): Promise<Article | null> {
  try {
    ...
    // SSR fetch failed (Docker Turbopack dev mode) — will fallback to client-side
    console.warn(`[getArticle] SSR fetch failed for "${slug}", using client fallback`);
    return null;
  }
}

// generateMetadata — works on Railway (SSR), graceful fallback in Docker dev
...
export default async function ArticleDetailPage({
  ...
  // === FALLBACK: SSR failed (Docker dev mode) → render client-side ===
  if (!article) {
    return <ClientArticleDetail slug={slug} />;
  }

  // === SSR PATH: Full server-rendered article with SEO ===
  ...
  // Review + Vehicle schema for car articles
  if (article.specs) {
    ...
    const reviewSchema: Record<string, unknown> = {
      ...
      "datePublished": article.created_at,
      "publisher": { "@type": "Organization", "name": "Fresh Motors" },
      "itemReviewed": vehicleSchema,
      "url": fullUrl,
    };

    if (article.average_rating > 0 && article.rating_count > 0) {
      reviewSchema["reviewRating"] = { "@type": "Rating", "ratingValue": article.average_rating.toFixed(1), "bestRating": "5", "worstRating": "1" };
      reviewSchema["aggregateRating"] = { "@type": "AggregateRating", "ratingValue": article.average_rating.toFixed(1), "bestRating": "5", "worstRating": "1", "ratingCount": article.rating_count };
    }
    ...
    if (article.price_usd && article.price_usd > 0 && article.show_price !== false) {
      jsonLdData.push({
        ...
        "description": article.summary,
        "offers": { "@type": "Offer", "priceCurrency": "USD", "price": article.price_usd, "availability": "https://schema.org/InStock", "url": fullUrl },
        ...(article.average_rating > 0 ? { "aggregateRating": { "@type": "AggregateRating", "ratingValue": article.average_rating.toFixed(1), "bestRating": "5", "ratingCount": article.rating_count } } : {})
      });
    }
  }
  ...
  return (
    <>
      ...
      <ViewTracker articleSlug={article.slug} articleId={article.id} articleTitle={article.title} categoryName={article.categories?.[0]?.name} />
      <ABImpressionTracker
      ...
      <main className="flex-1 bg-gray-50">
        ...
        <div className="relative h-[250px] sm:h-[350px] md:h-[500px] lg:h-[600px] xl:h-[650px] 2xl:h-[700px] max-h-[700px] w-full">
          <Image src={imageUrl} alt={article.title} fill className="object-cover" priority loading="eager" unoptimized />
        </div>
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-4xl mx-auto">
              ...
              <div className="flex flex-wrap gap-4 sm:gap-6 text-xs sm:text-sm text-gray-600 items-center">
                ...
                {(article.show_source !== false) && article.author_name ? (
                  <div className="flex items-center gap-2">
                    ...
                    {article.author_channel_url ? (
                      <a href={article.author_channel_url} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-800 transition-colors font-bold">
                        {article.author_name}
                      </a>
                    ) : (
                ...
                {article.rss_feed && (
                  <div className="flex items-center gap-2">
                    ...
                    {article.source_url && (
                      <a href={article.source_url} target="_blank" rel="noopener noreferrer" className="text-orange-600 hover:text-orange-800 transition-colors flex items-center gap-1">
                ...
                {(article.average_rating > 0 && article.rating_count > 0) && (
                  <ArticleRating initialRating={article.average_rating} initialCount={article.rating_count} />
                )}
              </div>
            ...
            <div className="mb-8">
              {article.categories && article.categories.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {article.categories.map((cat: { id: number; name: string; slug: string }) => (
                    <Link key={cat.id} href={`/categories/${cat.slug}`} className="inline-block bg-indigo-600 text-white px-4 py-2 rounded-full text-sm font-bold hover:bg-indigo-700 transition-colors">
                      {cat.name}
                    </Link>
                  ))}
                </div>
              ...
              <p className="text-base sm:text-lg md:text-xl text-gray-600 max-w-3xl">{article.summary}</p>

              {article.price_usd && article.show_price !== false && (
                <div className="mt-4"><PriceConverter priceUsd={article.price_usd} /></div>
              )}
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="space-y-8">
              ...
              {hasYoutubeVideo && (
                <div className="bg-white rounded-xl shadow-md p-4 sm:p-8">
                  ...
                  <div className="relative w-full aspect-video rounded-xl overflow-hidden shadow-lg">
                    <iframe src={youtubeEmbedUrl} title={article.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen loading="lazy" className="absolute inset-0 w-full h-full" />
                  </div>
                </div>
              )}
              ...
              <div className="flex justify-center"><FeedbackButton articleSlug={article.slug} /></div>
                ...
                <div className="bg-white rounded-xl shadow-md p-6">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {article.tag_names.map((tag: string) => (
                      <Link key={tag} href={`/articles?tag=${tag}`} className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-full hover:bg-indigo-100 transition-colors font-medium border border-indigo-100">
                        {tag}
                      </Link>
                    ))}
                  </div>
              ...
              {article.categories?.[0]?.slug && (
                <RelatedCarousel categorySlug={article.categories[0].slug} currentArticleSlug={article.slug} currentArticleId={article.id} />
              )}
