...
### ❌ "CRITICAL: sorry, too many clients already"

**Cause**: Railway PostgreSQL has a connection limit (~25). `conn_max_age=600` keeps connections open per worker → pool exhausted under load (e.g. admin page making many parallel API requests).  
...
**Also check**: avoid `Promise.all()` with many parallel API requests on admin pages. Use bulk endpoints instead.
